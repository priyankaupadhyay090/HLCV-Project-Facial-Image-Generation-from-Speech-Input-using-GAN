...
from network.model import G_NET, D_NET64, D_NET128, D_NET256, D_NET512, D_NET1024, INCEPTION_V3, MD_NET
from network.ied import Inception_V3_Model, Linear_Encoder
from torch.utils.tensorboard import FileWriter, summary
...
def load_network(gpus,num_batches):
    ...
    if NET_D != '':
        for i in range(len(netsD)):
            print('Load %s_%d.pth' % (NET_D, i))
            state_dict = torch.load('%s%d.pth' % (NET_D, i))
...
def define_optimizers(netG, netsD):
    ...
    for i in range(num_Ds):
        opt = optim.Adam(netsD[i].parameters(), lr=DISCRIMINATOR_LR, betas=(0.5, 0.999))
...
class condGANTrainer(object):
    def __init__(self, output_dir, data_loader, imsize):
        ...
        self.snapshot_interval = 2000 #Used in their cfg
    ...
    def train_MDnet(self, count):
        ...
        errMD_si = MD_LOSS*nn.CrossEntropyLoss()(real_logits2,real_labels.long())
        errMD_sa = MD_LOSS*nn.CrossEntropyLoss()(same_logits,same_labels.long())
        errMD_fa = MD_LOSS*nn.CrossEntropyLoss()(fake_logits2,fake_labels.long())
        errMD_wr = MD_LOSS*nn.CrossEntropyLoss()(wrong_logits2,wrong_labels.long())
    ...
    def train_Dnet(self, idx, count):
        ...
        if len(real_logits) > 1 and UNCOND_LOSS > 0:
            errD_real_uncond = UNCOND_LOSS * criterion(real_logits[1], real_labels)
            errD_wrong_uncond = UNCOND_LOSS * criterion(wrong_logits[1], real_labels)
            errD_fake_uncond = UNCOND_LOSS * criterion(fake_logits[1], fake_labels)
            #
    ...
    def train_Gnet(self, count):
        ...
        if COLOR_LOSS > 0:
            if self.num_Ds > 1:
                ...
                like_mu2 = COLOR_LOSS * nn.MSELoss()(mu1, mu2)
                like_cov2 = COLOR_LOSS * 5 * nn.MSELoss()(covariance1, covariance2)
            ...
            if self.num_Ds > 2:
                ...
                like_mu1 = COLOR_LOSS * nn.MSELoss()(mu1, mu2)
                like_cov1 = COLOR_LOSS * 5 * nn.MSELoss()(covariance1, covariance2)
    ...
    def train(self):
        ...
        if CONTENTCONSIST_LOSS>0 or SEMANTICONSIST_LOSS>0 or MD_LOSS>0:
            self.image_cnn = Inception_V3_Model()
        ...
        if MD_LOSS>0:        
            self.optimizerMD = optim.Adam(self.netMD.parameters(), lr=DISCRIMINATOR_LR, betas=(0.5, 0.999))
